...
| `pool_submissions_stats_v` ✅ | 풀 등록 제보 처리 통계 (0081) | status, mode, submission_count, avg_processing_days, oldest/newest_submitted_at | service_role 전용(`invoker=on`, base RLS 차단). 운영자 SLA 모니터링 |
| `schedule_submissions_stats_v` ✅ | 시간표 제보 처리 통계 (0081) | status, submission_count, unique_pools, unique_submitters, avg_processing_days | service_role 전용. nickname 기반 unique 제보자 집계 가능 |
| `terms_consent_v` ✅ | 약관별 현재 동의 현황 (0081) | terms_type, terms_version, action, user_count | `terms_agreement_current` 위 집계. 마케팅 동의자 수·재동의 진행률 |
| `notifications_kind_v` ✅ | 알림 kind별 발송·읽음률 (0081) | kind, total/read_count, read_rate, last_30d_count·last_30d_read_count, unique_recipients, last_sent_at | service_role 전용(`invoker=on` + anon/auth `revoke`). base 가 select_any 라 view 권한도 명시 차단 |
