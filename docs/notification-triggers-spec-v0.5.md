...
상대 행동 트리거 → 프로필:
  ...
  - invite_received 상태 1·4 (상대 초대·취소)
  - invite_sent 상태 2·3 (상대 수락·거절)
  - friend_schedule_overlap (상대 일정 등록)

본인 행동 / 시스템 처리 / 2명 이상 → 아이콘:
  - friend_request_rejected (본인 거절 이력) [친구]
  - invite_received 상태 2·3·5 (본인 응답·시스템 만료) [초대]
  - invite_sent 전체 상태 (본인 발송 / 시스템 처리) [초대]
  - 그 외 시스템 안내·운영자 발·자동 알림·환영 모두 아이콘
```
