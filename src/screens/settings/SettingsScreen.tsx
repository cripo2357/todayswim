...
// 친구 수영 일정 (Figma 179:4763) — prefs.mapFriendHorizon 3값.
// 2026-05-22 'off' 옵션 폐기 — 친구는 항상 표시, horizon 만 선택.
...
// 사람들 수영 일정 (Figma 179:4763) — prefs.mapPublicHorizon.
// 친구와 달리 'off' 유지 — 비친구 데이터라 안 보는 선택지 유효.
// 행 자체는 profileVis==='public'일 때만 노출(친구공개면 미표시).
const MAP_PUBLIC_VALUE: Record<MapPublicHorizon, string> = {
  d1: MAP_FRIEND_VALUE.d1,
  h12: MAP_FRIEND_VALUE.h12,
  h6: MAP_FRIEND_VALUE.h6,
  off: '표시 안함',
};
const MAP_PUBLIC_OPTIONS: Option<MapPublicHorizon>[] = [
  { value: 'off', label: MAP_PUBLIC_VALUE.off },
  { value: 'h6', label: MAP_PUBLIC_VALUE.h6 },
  { value: 'h12', label: MAP_PUBLIC_VALUE.h12 },
  { value: 'd1', label: MAP_PUBLIC_VALUE.d1 },
];
// "내 수영 일정·레슨" 행 폐기 (2026-05-22) — profileVisibility 와 의미 중복
// (둘 다 'friends' / 'public' 2값 + 일정·레슨 노출 범위는 프로필 공개범위와
// 동일하게 따라감). prefs.myScheduleVisibility 및 set 함수도 prefs.ts 에서
// 제거. AsyncStorage 옛 키는 hydrate 가 1회 removeItem.
// 친구 신청 받기
